"""
SF Bay Area + US-Remote Job Scraper (Jackie)
...
# Bare `supervisor`, `team lead`, `team leader`, `manager`, `coordinator`,
# and `specialist` are deliberately excluded — a US-remote Indeed sweep on a
# bare "supervisor" is dominated by warehouse / shift / production roles. Only
# the compound forms below are allow-listed. Multi-word phrases are
# substring-matched; single words ("escalation", "onboarding", "enablement",
# "revops") are word-bounded. Engineering/licensed-clinical/sales-quota/
# industrial titles are vetoed separately by EXCLUDED_DOMAIN_RE.
KEYWORDS = [
    # ---- Operations ----
    "operations manager", "operations lead", "operations supervisor",
    "operations team lead", "operations coordinator", "operations specialist",
    "operations analyst", "business operations", "clinical operations",
    "care operations", "healthcare operations", "health operations",
    "patient operations", "service operations", "support operations",
    "contact center", "call center", "call center supervisor",
    "patient services supervisor", "workforce management",
    "quality assurance", "quality manager", "quality lead",
    ...
    "customer care", "support team lead", "support manager",
    "support supervisor", "support lead", "member support",
    "member experience", "member services", "client services",
    "technical support manager", "technical support lead", "escalation",
    "escalations",
    # ---- Patient / care (healthtech) ----
    "patient support", "patient services", "patient experience",
    "patient success", "patient navigator", "patient advocate",
    "patient engagement", "patient access", "care coordinator",
    "care coordination", "care navigator", "care team", "care manager",
    "case manager", "clinical support", "clinical care", "care delivery",
    "medication adherence", "pharmacy operations", "pharmacy support",
    "population health", "health coach", "care advocate",
    "enrollment specialist", "intake coordinator", "health program",
    "community health",
    # ---- Training / enablement ----
    "training manager", "training lead", "training specialist",
    "training coordinator", "learning and development",
    "learning & development", "enablement", "onboarding",
    "instructional designer", "quality assurance specialist",
    # ---- Program / project / people ----
    "program manager", "program coordinator", "program lead",
    "project manager", "project coordinator", "implementation manager",
    "implementation specialist", "implementation lead", "people operations",
    "hr coordinator", "hr generalist", "people partner", "office manager",
    # ---- Tech-company ops ----
    "product operations", "product ops", "solutions specialist",
    "customer onboarding", "revenue operations", "revops", "sales operations",
    "business analyst", "operations associate", "trust and safety",
    "trust & safety", "community operations", "vendor manager",
    "strategy and operations", "strategy & operations", "launch manager",
    "market manager", "account manager", "account management",
]

# Seconds to wait between API probes — keeps us polite
REQUEST_DELAY = 0.3

# Curated-employer digest should only contain reliably fresh roles.
FRESH_JOB_LOOKBACK = timedelta(hours=24)

# Acquisition lookbacks stay bounded; accepted listings have no age ceiling.
MAX_POSTING_AGE_DAYS = None  # Listings do not expire by age.

# Senior-manager and executive titles are excluded everywhere. Bare "senior",
# "staff", "lead", "manager", and "supervisor" are ALLOWED — Jackie leads a
# 50-person support team, so Operations Manager / Support Team Lead /
# Senior Care Coordinator / Staff Development Coordinator are target titles.
# "Senior Manager" and above (director, VP, chief, head of, general/regional
# manager) are the veto line.
EXCLUDED_SENIORITY_RE = re.compile(
    r'\b(principal|distinguished|founding|director|vice president|vp|svp|evp|'
    r'chief|head\s+of|executive director|associate director|managing director)\b'
    r'|\b(senior|sr\.?)\s+manager\b'
    r'|\b(general|group|regional|national|district)\s+manager\b',
    re.IGNORECASE)

# Domain veto (2026-09-03): titles that match a KEYWORDS lane by substring but
# belong to a track Jackie can't or won't take. Keep in sync with
# EXCLUDED_DOMAIN_RE in triage.html.
#  - licensed clinical roles (she is not RN/PharmD/therapist) — "clinician" is
#    deliberately NOT here: "Clinician Support Lead" is a real ops title;
#  - engineering / data / technical-program roles;
#  - quota-carrying sales (AE/SDR) — "account manager" stays allowed;
#  - industrial / retail / hospitality operations (warehouse, shift, hotel…),
#    which dominate a nationwide "operations manager" sweep.
EXCLUDED_DOMAIN_RE = re.compile(
    r'\b(rn|lpn|lvn|nurse|nursing|pharmacist|pharmd|physician|md|np|pa-c|'
    ...
    r'plant|shift|construction|logistics|fleet|trucking|restaurant|hotel|'
    r'hospitality|retail|store manager|branch|hvac|janitorial|security guard)\b',
    re.IGNORECASE)
...
def is_target_role(title: str) -> bool:
    if EXCLUDED_SENIORITY_RE.search(title) or EXCLUDED_DOMAIN_RE.search(title):
        return False
    return bool(_KEYWORD_RE.search(title))


# State confirm for ambiguous city names (used by _metro_confirmed).
_STATE_CONFIRM = {
...
# ---- SF Bay Area (Jackie is in San Francisco) ----
BAY_AREA_LOCATIONS = [
    "bay area",
...
# Bay Area only — US-remote roles come in through INCLUDE_REMOTE_US below.
WATCH_METROS = [
...
# Jackie's search is SF Bay Area + US-remote (ops/support leadership roles at
# digital-health companies are often remote-first). Remote rows arrive mainly
# via the JobSpy/Indeed "Remote" location — see JOBSPY_LOCATIONS.
INCLUDE_REMOTE_US = True


def is_watch_location(location: str) -> bool:
    """Geo gate for the location-scoped watchers: SF Bay Area + US-remote.
    Uses the confirmed metro gate — its callers see nationwide location
    strings, where bare "richmond"/"newark"/"dublin" substrings would
    otherwise pass for out-of-state cities."""
    return (
...
CURATED_HOLLYWOOD = [
    # Emptied for Jackie (2026-09-03). Phase 2 repoints this pipeline at
    # curated digital-health employers (Omada, Included Health, Lyra, …).
]
...
LINKEDIN_SEARCH_TERMS = [
    ...
    # produce for government boards (county/state health programs) come first.
    "program coordinator",
    "health program",
    "operations manager",
    "project manager",
...
LINKEDIN_HOLLYWOOD_LOOKBACK_SECONDS = 86400 # 24h — curated-employer feed (dormant until Phase 2)

# Guest-endpoint geo scopes as (display name, LinkedIn geoId) pairs.
# 90000084 (SF Bay Area) carried over from the parent repos.
#
# Bay geo ONLY — do not add a nationwide ("United States", 103644278) entry
# for remote coverage: verified live 2026-09-03 that the guest endpoint
# IGNORES the f_WT=2 remote work-type filter (identical on-site results with
# and without it), so a US-wide query just burns the 75-card-per-term budget
# on roles the location gate rejects. US-remote rows come from the JobSpy /
# Indeed "Remote" location instead (JOBSPY_LOCATIONS).
LINKEDIN_LOCATIONS = [
    ("San Francisco Bay Area", "90000084"),
]

# Curated-employer allowlist used by the LinkedIn-side filter. Matching is an
# exact normalized-name comparison (_normalize_company_name), so spell names
# the way LinkedIn renders them. Emptied for Jackie until Phase 2 (healthtech).
...
# Location scopes for the jobspy-backed sources (Indeed, ZipRecruiter + Google).
# 50mi from SF covers the Peninsula, East Bay, and South Bay; "Remote" is
# Indeed's nationwide remote scope. The central post-fetch policy is
# authoritative.
JOBSPY_LOCATIONS = [
    ("San Francisco, CA", 50),
    # "Remote" as a JobSpy location returns Indeed rows labeled "Remote, US"
    # (verified 2026-09-03, 20/20 rows passed is_remote_us with hours_old
    # intact). Do NOT use jobspy's is_remote=True — it is mutually exclusive
    # with hours_old and silently breaks the time filter.
    ("Remote", 50),
]
...
def scrape_indeed_recent() -> list:
    """Indeed roles posted in the last INDEED_LOOKBACK_HOURS, SF Bay Area + US Remote."""
    ...
    for (location, distance), term in itertools.product(JOBSPY_LOCATIONS, LINKEDIN_SEARCH_TERMS):
        ...
        try:
            ...
            df = _jobspy_fetch_with_retry(
                ...
                search_term=term,
                google_search_term=(
                    f"remote {term} jobs since yesterday"
                    if location.lower() == "remote"
                    else f"{term} jobs near {location} since yesterday"
                ),
                location=location,
                distance=distance,
...
FILTER_STAT_KEYS = ("company", "seniority", "domain", "role", "location", "stale")
...
def _filter_job_observations(jobs: list[dict], *, default_feed: str):
    ...
    for original in jobs:
        ...
        if is_excluded_company(str(job.get("company", "") or "")):
            ...
            reason = "seniority"
        elif EXCLUDED_DOMAIN_RE.search(title):
            reason = "domain"
...
ALL_JOBS_PRUNE_DAYS = None  # Retain the cumulative master indefinitely.
...
def _merge_into_all_jobs(observed_jobs: list, rejected_observations: list | None = None) -> int:
    ...
    every retained listing without an age cutoff. Returns count added.
    """
...
def save_linkedin_results(jobs: list):
    save_jobs_output(
        ...
        title="🔥 LinkedIn — Ops / Support / Care Roles (SF Bay Area + US Remote)",
        subtitle=f"SF Bay Area + US Remote · last {LINKEDIN_LOOKBACK_SECONDS // 3600}h",
        accent="#3b82f6",
...
def save_indeed_results(jobs: list):
    save_jobs_output(
        ...
        title="🟦 Indeed — Ops / Support / Care Roles (SF Bay Area + US Remote)",
        subtitle=f"SF Bay Area + US Remote · last {INDEED_LOOKBACK_HOURS}h",
        accent="#2557a7",
        empty_message="No new roles since the last run.",
...
def save_boards_results(jobs: list):
    save_jobs_output(
        ...
        title="🟪 ZipRecruiter + Google — Ops / Support / Care Roles (SF Bay Area + US Remote)",
        subtitle=f"SF Bay Area + US Remote · last {BOARDS_LOOKBACK_HOURS}h",
        accent="#7c5cff",
        empty_message="No new roles since the last run.",
...
def save_hollywood_results(jobs: list):
    save_jobs_output(
        ...
        title="🏥 Healthtech — Curated Employers (direct ATS + LinkedIn allowlist)",
        subtitle=f"Curated digital-health employers · last {LINKEDIN_HOLLYWOOD_LOOKBACK_SECONDS // 3600}h",
        accent="#e879f9",
        empty_message="No new curated roles since the last run.",
...
{body}
<div class="foot">Auto-generated by <a href="https://github.com/ernestod1998/Jackie_Job_Scraper">Jackie_Job_Scraper</a></div>
</body></html>"""
...
def save_results(jobs: list):
    ...
    lines = [
        "# 🏥 Fresh Curated-Employer Job Listings",
        f"*Last updated: {timestamp}*\n",
    ...
    with open(os.path.join(SCRIPT_DIR, "jobs.html"), "w") as f:
        f.write(_render_jobs_html(
            title="🏥 Fresh Curated-Employer Job Listings",
            subtitle="Curated employers · posted in the last 24 hours",
            timestamp=timestamp,
            jobs=jobs,
...
# location predicate (is_watch_location — SF Bay Area + US Remote), so they
# follow whatever KEYWORDS / BAY_AREA_LOCATIONS the maintainer sets — no
...
def scrape_governmentjobs_recent() -> list:
    ...
    repo's watch locations (SF Bay Area + US Remote) with is_watch_location()."""
...
def save_registry_results(result: dict) -> None:
    save_jobs_output(
        ...
        title="🗃 Direct ATS Registry — Ops / Support / Care Roles",
...
REFILTER_RENDER_CONFIG = {
    "hollywood_jobs": ("🏥 Healthtech — Curated Employers", "Curated digital-health employers (dormant until Phase 2)", "#e879f9", "No new curated roles since the last run."),
    "linkedin_jobs": ("🔥 LinkedIn — Ops / Support / Care Roles (SF Bay Area + US Remote)", "SF Bay Area + US Remote", "#3b82f6", "No new roles since the last run."),
    "indeed_jobs": ("🟦 Indeed — Ops / Support / Care Roles (SF Bay Area + US Remote)", "SF Bay Area + US Remote", "#2557a7", "No new roles since the last run."),
    "boards_jobs": ("🟪 ZipRecruiter + Google — Ops / Support / Care Roles", "SF Bay Area + US Remote", "#7c5cff", "No new roles since the last run."),
    ...
    "registry_jobs": ("🗃 Direct ATS Registry — Ops / Support / Care Roles", "Verified public ATS boards", "#f59e0b", "No eligible registry roles in this shard."),
}
...
if __name__ == "__main__":
    ...
    print(f"\n📍 SF Bay Area + US Remote filter: {before} → {len(all_jobs)} roles")
